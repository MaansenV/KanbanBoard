...
- `src/App.tsx` composes the board shell, filters, modals, drag-and-drop actions, and agent dispatch.
...
- `server/dispatch-store.mjs` manages agent registration and task dispatch persistence.
...
| `src/components/` | Feature UI split into board, layout, modal, stats, dispatch, MCP, and primitive subtrees. | [src/components/codemap.md](src/components/codemap.md) |
